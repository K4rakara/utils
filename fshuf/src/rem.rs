...
pub(crate) fn rem(prefix: Option<String>) {
		...
		Some(v) => {
			if pwd.join(".fshuf").exists() {
				...
				match fs::read_to_string(pwd.join(".fshuf")) {
					Ok(v2) => {
						if v != v2 {
							println!("\u{001b}[38;5;3mWarning:\u{001b}[0m The passed pattern \"{passed}\" and the stored pattern \"{stored}\" do not match.",
								passed = v,
								stored = v2)
						}
						v
	...
	match fs::read_dir(&pwd) {
								...
								if &file_name_string != ".fshuf" {
									...
									if file_name_string.len() >= prefix_string.len() {
										...
										if prefix.can_output(&file_prefix) {
											...
											// Add the old and new filename to the vec of files to
                                            // rename.
		...
		Err(e) => {
			println!("\u{001b}[38;5;1mError:\u{001b}[0m An uncatchable I/O error occured. Details:\n{err}",
				err = e);
			exit(1);
		}
	}
